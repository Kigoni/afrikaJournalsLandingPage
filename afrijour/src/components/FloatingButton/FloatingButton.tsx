...
const FloatingButton: React.FC<FloatingButtonProps> = ({ onClick }) => {
  ...
  //   (e: React.FormEvent) => {
  //     e.preventDefault();
  //     console.log("Form name submitted:", formData);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://aphrc.site/journal_api/api/feedback/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          id: Date.now(), // Generate a unique ID (optional, if not managed by the server)
          name: formData.name,
